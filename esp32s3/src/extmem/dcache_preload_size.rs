...
impl DCACHE_PRELOAD_SIZE_R {
    ...
    pub(crate) fn new(bits: u16) -> Self {
        DCACHE_PRELOAD_SIZE_R(crate::FieldReader::new(bits))
    }
}
...
pub struct DCACHE_PRELOAD_SIZE_W<'a> {
    w: &'a mut W,
}
impl<'a> DCACHE_PRELOAD_SIZE_W<'a> {
...
impl W {
    #[doc = "Bits 0:15 - The bits are used to configure the length for preload operation. The bits are the counts of cache block. It should be combined with DCACHE_PRELOAD_ADDR_REG.."]
    #[inline(always)]
    pub fn dcache_preload_size(&mut self) -> DCACHE_PRELOAD_SIZE_W {
        DCACHE_PRELOAD_SIZE_W { w: self }
    }
